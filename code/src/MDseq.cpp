...
int main()
{
    ...
    fprintf(afp," --------------   -----------        ---------------   --------------   ---------------   ------------   -----------\n");
    fprintf(afp,"  %8.4e  %15.12f       %15.12f     %10.12f       %10.12f        %10.5e         %i\n",i*dt*timefac,Tavg,Pavg,gc,Z,Vol*VolFac,N);
...
double MeanSquaredVelocity() { 
    ...
    double mean_squared_velocity = v2_sum / N;
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    for (int i=0; i<N; i++) {
            ...
            r[i][0] += v[i][0]*dt + c*a[i][0]*dt;            
            v[i][0] += c*a[i][0];

            
            r[i][1] += v[i][1]*dt + c*a[i][1]*dt;            
            v[i][1] += c*a[i][1];

            
            r[i][2] += v[i][2]*dt + c*a[i][2]*dt;            
            v[i][2] += c*a[i][2];
